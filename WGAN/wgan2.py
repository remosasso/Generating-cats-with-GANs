...
fp = glob.glob("/home/s2936860/CAT-GAN/mega-cat-dataset/*")
path_samples = "/data/s2936860/WGAN-RR-1"
...
class WGAN(object):
    ...
    def next_batch(num=64, data=fp):
        idxx = np.arange(0, len(data)-1)
        np.random.shuffle(idxx)
        idxx = idxx[:64]
        data_shuffle = [imread(data[i]) for i in idxx]
    ...
    def sample_z(self, num=64):
        return np.random.uniform(-0., 1., size=[num, self.flags.z_dim])
    ...
    def plots(self, imgs_, iter_time, save_file, cur_time):
        ...
        imgs = [imgs[idx] for idx in range(len(imgs))]
        for img in imgs:
        ...
        ctr = 0        
        for img in imgs:
          plt.imsave(path_samples + "/cat_epoch" +str(iter_time)+'_n'+str(ctr)+'.png', img)
          ctr += 1
